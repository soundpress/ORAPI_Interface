	...
	static function serviceCard($rec)
	{
		...
		if ($r['SchedulesDays'])
			foreach ((array)$r['SchedulesDays'] as $i=>$day)
				$ss[] = "<span class='weekday'>{$day}</span> {$r['SchedulesOpen'][$i]}-{$r['SchedulesClose'][$i]}";
		$r['Schedules']	= implode('<br/>', $ss);
		
		$r = array_diff_key($r, array_fill_keys(['Address2', 'Address3', 'Address4', 'SchedulesDays', 'SchedulesOpen', 'SchedulesClose'], 1));
		...
		foreach ($mm as $rec)
			if ($rec['lat'] && $rec['lon'])
			{
				$rr[] = implode("\t", [
					...
					'title' => "<a href=\"service.php?id={$rec['id']}\">{$rec['service']}" .
